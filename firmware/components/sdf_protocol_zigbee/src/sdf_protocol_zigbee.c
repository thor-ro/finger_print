...
void esp_zb_app_signal_handler(esp_zb_app_signal_t *signal_struct)
{
    ...
    case ESP_ZB_BDB_SIGNAL_STEERING:
        if (err_status == ESP_OK) {
            ...
            ESP_LOGW(TAG, "Network steering failed (status=%s), scheduling retry", esp_err_to_name(err_status));
...
static bool sdf_zigbee_set_attr_u8(uint16_t cluster_id, uint16_t attr_id, uint8_t value)
{
    if (!esp_zb_lock_acquire(pdMS_TO_TICKS(1000))) {
        ESP_LOGW(TAG, "Timeout acquiring Zigbee lock for cluster=0x%04X attr=0x%04X", cluster_id, attr_id);
        return false;
    }
    ...
    if (status != ESP_ZB_ZCL_STATUS_SUCCESS) {
        ESP_LOGW(
            TAG,
            "Failed to set Zigbee attribute cluster=0x%04X attr=0x%04X status=0x%02X",
            cluster_id,
            attr_id,
            (unsigned)status);
        return false;
...
static bool sdf_zigbee_set_attr_u16(uint16_t cluster_id, uint16_t attr_id, uint16_t value)
{
    if (!esp_zb_lock_acquire(pdMS_TO_TICKS(1000))) {
        ESP_LOGW(TAG, "Timeout acquiring Zigbee lock for cluster=0x%04X attr=0x%04X", cluster_id, attr_id);
        return false;
...
static esp_err_t sdf_zigbee_handle_door_lock_command(const esp_zb_zcl_door_lock_lock_door_message_t *message)
{
    ...
    if (message->info.status != ESP_ZB_ZCL_STATUS_SUCCESS) {
        ESP_LOGW(TAG, "Received invalid Door Lock command status=0x%02X", (unsigned)message->info.status);
        return ESP_ERR_INVALID_ARG;
    }
...
static esp_err_t sdf_zigbee_register_endpoint(void)
{
    ...
    esp_zb_cluster_list_t *cluster_list = esp_zb_door_lock_clusters_create(&door_lock_cfg);
    ESP_RETURN_ON_FALSE(cluster_list != NULL, ESP_ERR_NO_MEM, TAG, "Failed to create Door Lock cluster list");

    esp_zb_attribute_list_t *door_lock_cluster = esp_zb_cluster_list_get_cluster(
        cluster_list,
        ESP_ZB_ZCL_CLUSTER_ID_DOOR_LOCK,
        ESP_ZB_ZCL_CLUSTER_SERVER_ROLE);
    ESP_RETURN_ON_FALSE(door_lock_cluster != NULL, ESP_FAIL, TAG, "Door Lock cluster not found");
    ...
    ESP_RETURN_ON_FALSE(power_cluster != NULL, ESP_ERR_NO_MEM, TAG, "Failed to create Power Config cluster");
    ...
    ESP_RETURN_ON_FALSE(ep_list != NULL, ESP_ERR_NO_MEM, TAG, "Failed to create endpoint list");
...
static void sdf_zigbee_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "Zigbee stack started (Door Lock endpoint %u)", (unsigned)SDF_ZIGBEE_ENDPOINT);
...
esp_err_t sdf_protocol_zigbee_init(void)
{
    ...
    if (s_state.lock == NULL) {
        ...
        ESP_RETURN_ON_FALSE(s_state.lock != NULL, ESP_ERR_NO_MEM, TAG, "Failed to create Zigbee mutex");
    }
    ...
    BaseType_t task_ok = xTaskCreate(
        sdf_zigbee_task,
        SDF_ZIGBEE_TASK_NAME,
        SDF_ZIGBEE_TASK_STACK,
        NULL,
        SDF_ZIGBEE_TASK_PRIORITY,
        &s_state.task);
...
esp_err_t sdf_protocol_zigbee_set_command_handler(sdf_protocol_zigbee_command_cb cb, void *ctx)
{
    if (s_state.lock == NULL) {
        s_state.lock = xSemaphoreCreateMutex();
        if (s_state.lock == NULL) {
...
esp_err_t sdf_protocol_zigbee_update_alarm_mask(uint16_t alarm_mask)
{
    ...
    return sdf_zigbee_set_attr_u16(
               ESP_ZB_ZCL_CLUSTER_ID_DOOR_LOCK,
               ESP_ZB_ZCL_ATTR_DOOR_LOCK_ALARM_MASK_ID,
               alarm_mask)
               ? ESP_OK
