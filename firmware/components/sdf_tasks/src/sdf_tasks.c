...
#define SDF_POWER_CHECKIN_INTERVAL_DEFAULT_MS CONFIG_SDF_POWER_CHECKIN_INTERVAL_MS
#define SDF_POWER_CHECKIN_INTERVAL_MIN_MS 1000u
#define SDF_POWER_CHECKIN_INTERVAL_MAX_MS 600000u
#define SDF_POWER_IDLE_BEFORE_SLEEP_DEFAULT_MS CONFIG_SDF_POWER_IDLE_BEFORE_SLEEP_MS
...
#define SDF_POWER_BATTERY_REPORT_DEFAULT_MS CONFIG_SDF_POWER_BATTERY_REPORT_INTERVAL_MS
#define SDF_POWER_BATTERY_DEFAULT_PERCENT CONFIG_SDF_POWER_BATTERY_DEFAULT_PERCENT
...
static sdf_power_wake_reason_t sdf_tasks_map_wakeup_reason(esp_sleep_wakeup_cause_t cause)
{
    switch (cause) {
...
static const char *sdf_tasks_wakeup_reason_name(sdf_power_wake_reason_t reason)
{
    switch (reason) {
...
static void sdf_tasks_notify_wakeup(
    const sdf_power_manager_config_t *config,
...
static void sdf_tasks_sleep_once(const sdf_power_manager_config_t *config)
{
    ...
    esp_sleep_enable_timer_wakeup((uint64_t)config->checkin_interval_ms * 1000ULL);

    if (sdf_tasks_gpio_valid(config->fingerprint_wake_gpio)) {
        esp_err_t wake_err = sdf_tasks_configure_fingerprint_wakeup(config->fingerprint_wake_gpio);
        if (wake_err != ESP_OK) {
            ESP_LOGW(TAG, "Failed to configure fingerprint wake GPIO: %s", esp_err_to_name(wake_err));
        }
    }
    ...
    sdf_power_wake_reason_t reason = sdf_tasks_map_wakeup_reason(esp_sleep_get_wakeup_cause());
    ESP_LOGI(TAG, "Woke from light sleep (%s)", sdf_tasks_wakeup_reason_name(reason));
...
static void sdf_tasks_task(void *arg)
{
    ...
    while (true) {
        ...
        if (now_us >= next_battery_report_us) {
            ...
            if (xSemaphoreTake(s_state.lock, pdMS_TO_TICKS(SDF_POWER_LOCK_WAIT_MS)) == pdTRUE) {
                ...
                s_state.next_battery_report_us =
                    now_us + ((int64_t)config_snapshot.battery_report_interval_ms * 1000LL);
                xSemaphoreGive(s_state.lock);
            }
        ...
        if (allow_sleep && (now_us - last_activity_us) < ((int64_t)config_snapshot.idle_before_sleep_ms * 1000LL)) {
            allow_sleep = false;
        }
...
esp_err_t sdf_tasks_init_power_manager(const sdf_power_manager_config_t *config)
{
    if (config == NULL ||
        config->checkin_interval_ms < SDF_POWER_CHECKIN_INTERVAL_MIN_MS ||
        config->checkin_interval_ms > SDF_POWER_CHECKIN_INTERVAL_MAX_MS ||
        config->idle_before_sleep_ms == 0 ||
        config->post_wake_guard_ms == 0 ||
    ...
    esp_err_t zigbee_cfg_err = sdf_protocol_zigbee_set_checkin_interval_ms(config->checkin_interval_ms);
    ...
    if (zigbee_cfg_err == ESP_ERR_INVALID_STATE) {
        ESP_LOGW(TAG, "Zigbee check-in update ignored because Zigbee stack is already running");
    }

    BaseType_t task_ok = xTaskCreate(
        sdf_tasks_task,
        SDF_POWER_TASK_NAME,
        SDF_POWER_TASK_STACK,
        NULL,
        SDF_POWER_TASK_PRIORITY,
        &s_state.task);
    if (task_ok != pdPASS) {
        return ESP_FAIL;
    }
    ...
    ESP_LOGI(
        TAG,
        "Power manager started (checkin=%u ms, idle_sleep=%u ms, wake_gpio=%d, light_sleep=%u)",
...
esp_err_t sdf_tasks_set_checkin_interval_ms(uint32_t checkin_interval_ms)
{
    ...
    esp_err_t err = sdf_protocol_zigbee_set_checkin_interval_ms(checkin_interval_ms);
    if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
        return err;
    }

    if (s_state.lock != NULL &&
        xSemaphoreTake(s_state.lock, pdMS_TO_TICKS(SDF_POWER_LOCK_WAIT_MS)) == pdTRUE) {
