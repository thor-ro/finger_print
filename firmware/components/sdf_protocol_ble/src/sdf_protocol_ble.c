...
static int sdf_nuki_build_unencrypted_message(
    uint16_t command,
...
static int sdf_nuki_secretbox_encrypt(
...
static int sdf_nuki_secretbox_decrypt(
...
static int sdf_nuki_build_pdata(
    uint32_t auth_id,
    uint16_t command,
    const uint8_t *payload,
    size_t payload_len,
    ...
    size_t *out_len)
{
    ...
    uint16_t crc = sdf_nuki_crc_ccitt(out, SDF_NUKI_PDATA_HEADER_LEN + payload_len);
    ...
    size_t *out_len)
{
    ...
    res = sdf_nuki_secretbox_encrypt(
        pdata,
        pdata_len,
        nonce,
        shared_key,
...
static int sdf_nuki_build_encrypted_message(
    sdf_nuki_client_t *client,
    uint16_t command,
    const uint8_t *payload,
    size_t payload_len,
    ...
    void *message_ctx)
{
    ...
    uint16_t crc_expected = sdf_nuki_le16_read(
        client->pd_buf + plaintext_len - 2);
    uint16_t crc_actual = sdf_nuki_crc_ccitt(
        client->pd_buf, plaintext_len - 2);
    ...
    size_t payload_len)
{
    ...
    if (client->send_unencrypted_cb != NULL) {
        return client->send_unencrypted_cb(
            client->send_unencrypted_ctx, message, message_len);
    }

    return client->send_encrypted_cb(
        client->send_encrypted_ctx, message, message_len);
}
    ...
    size_t len)
{
    ...
    for (;;) {
        if (client->rx_expected == 0) {
            ...
            uint16_t msg_len = sdf_nuki_le16_read(
                client->rx_buf + SDF_NUKI_NONCE_LEN + 4);
            size_t total_len = SDF_NUKI_ADATA_LEN + msg_len;
            if (msg_len < SDF_NUKI_SECRETBOX_OVERHEAD || total_len > sizeof(client->rx_buf)) {
                sdf_nuki_client_reset_rx(client);
                return SDF_NUKI_RESULT_ERR_TOO_LARGE;
            }
    ...
    const uint8_t nonce_nk[SDF_NUKI_CHALLENGE_NONCE_LEN])
{
    ...
    uint8_t payload[1 + 4 + 1 + SDF_NUKI_NAME_SUFFIX_MAX + SDF_NUKI_CHALLENGE_NONCE_LEN];
    size_t offset = 0;
    ...
    const uint8_t nonce_nk[SDF_NUKI_CHALLENGE_NONCE_LEN])
{
    ...
    int res = sdf_nuki_build_encrypted_message(
        client,
        SDF_NUKI_CMD_SIMPLE_LOCK_ACTION,
        payload,
        offset,
        message,
        sizeof(message),
    ...
    uint8_t out[32])
{
    ...
    if (mbedtls_md_hmac(info, key, SDF_NUKI_SHARED_KEY_LEN, data, data_len, out) != 0) {
        return SDF_NUKI_RESULT_ERR_CRYPTO;
    }
