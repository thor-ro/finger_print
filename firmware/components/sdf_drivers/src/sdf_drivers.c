...
{
    return user_id >= SDF_FINGERPRINT_USER_ID_MIN && user_id <= SDF_FINGERPRINT_USER_ID_MAX;
}

static esp_err_t sdf_fingerprint_uart_read_exact(
    int uart_port,
    uint8_t *buffer,
    size_t expected_len,
    uint32_t timeout_ms)
...
static esp_err_t sdf_fingerprint_send_command_locked(
    uint8_t cmd,
    uint8_t p1,
    uint8_t p2,
    uint8_t p3,
    uint8_t response[SDF_FP_FRAME_LEN])
{
    ...
    int written = uart_write_bytes((uart_port_t)s_state.uart_port, request, sizeof(request));
    if (written != (int)sizeof(request)) {
        return ESP_FAIL;
    }

    esp_err_t err = sdf_fingerprint_uart_read_exact(
        s_state.uart_port,
        response,
        SDF_FP_FRAME_LEN,
...
static sdf_fingerprint_op_result_t sdf_fingerprint_map_ack_code(uint8_t ack_code)
{
    switch (ack_code) {
...
esp_err_t sdf_fingerprint_driver_init(const sdf_fingerprint_driver_config_t *config)
{
    if (config == NULL ||
        config->uart_port < 0 ||
    ...
    if (xSemaphoreTake(s_state.lock, pdMS_TO_TICKS(SDF_FP_MUTEX_WAIT_MS)) != pdTRUE) {
        return ESP_ERR_TIMEOUT;
    }
    ...
    ESP_LOGI(
        TAG,
        "Fingerprint UART initialized (port=%d, baud=%u, tx=%d, rx=%d)",
        config->uart_port,
        (unsigned)config->baud_rate,
        config->tx_pin,
        config->rx_pin);
...
sdf_fingerprint_op_result_t sdf_fingerprint_driver_match_1n(sdf_fingerprint_match_t *match)
{
    ...
    esp_err_t err = sdf_fingerprint_send_command_locked(SDF_FP_CMD_MATCH_1_N, 0, 0, 0, response);
    xSemaphoreGive(s_state.lock);

    if (err != ESP_OK) {
        return err == ESP_ERR_TIMEOUT ? SDF_FINGERPRINT_OP_TIMEOUT : SDF_FINGERPRINT_OP_PROTOCOL_ERROR;
    }
...
sdf_fingerprint_op_result_t sdf_fingerprint_driver_enroll_step(
    sdf_fingerprint_enroll_step_t step,
    uint16_t user_id,
    uint8_t permission)
{
    if (!sdf_fingerprint_user_id_valid(user_id) || permission < 1u || permission > 3u || s_state.lock == NULL) {
        return SDF_FINGERPRINT_OP_BAD_ARG;
    }
...
sdf_fingerprint_op_result_t sdf_fingerprint_driver_control_led(
    const sdf_fingerprint_led_command_t *cmd)
{
...
sdf_fingerprint_op_result_t sdf_fingerprint_driver_delete_user(uint16_t user_id)
{
...
sdf_fingerprint_op_result_t sdf_fingerprint_driver_delete_all_users(void)
{
    ...
    uint8_t response[SDF_FP_FRAME_LEN] = {0};
    esp_err_t err = sdf_fingerprint_send_command_locked(SDF_FP_CMD_DELETE_ALL_USERS, 0x00, 0x00, 0x00, response);
    xSemaphoreGive(s_state.lock);
