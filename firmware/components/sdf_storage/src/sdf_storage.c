...
static esp_err_t sdf_storage_validate_security_policy(void)
{
    ...
    if (!s_security_status.nvs_encryption_enabled) {
        if (s_security_status.require_encrypted_nvs) {
            ESP_LOGE(TAG, "Encrypted NVS is required, but CONFIG_NVS_ENCRYPTION is disabled");
    ...
    if (keys_partition == NULL) {
        ESP_LOGE(TAG, "NVS encryption enabled, but no nvs_keys partition was found");
    ...
    if (read_err == ESP_ERR_NVS_KEYS_NOT_INITIALIZED ||
        ...
        ESP_LOGW(TAG, "NVS key partition present, but key config is not readable yet: %s", esp_err_to_name(read_err));
    } else {
        ESP_LOGW(TAG, "Unable to read NVS security config: %s", esp_err_to_name(read_err));
    }
...
esp_err_t sdf_storage_init(void)
{
    ...
    if (s_security_status.nvs_encryption_enabled && s_security_status.nvs_keys_partition_present) {
...
{
    if (!s_security_status.nvs_encryption_enabled && s_security_status.require_encrypted_nvs) {
        return false;
    }

    if (!s_security_status.nvs_keys_partition_present && s_security_status.require_encrypted_nvs) {
        return false;
    }

    if (s_security_status.require_encrypted_nvs && !s_security_status.nvs_keys_accessible) {
        return false;
    }
