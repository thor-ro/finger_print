...
First official release. Everything below `1.0.0` was pre-release; this is the
first version tagged in git, so it is also the first image whose `PROJECT_VER`
(`git describe --tags`) reads as a version rather than a bare commit hash.
...
### Fixed
- **The fingerprint sensor could never answer (release blocker).** `CONFIG_SDF_FP_BAUD_RATE` defaulted to 115200, but the sensor module is fixed at **19200 bps 8N1** (`doc/UART_Fingerprint_Sensor_User_Manual_en.md`) and has no baud-change command, so every transaction timed out and the boot probe always failed. The default is now 19200 in `sdkconfig.defaults` and in the Kconfig entry, whose help text records that a wrong value shows up only as a boot-time `Sensor probe FAILED`. The probe's failure log now prints the baud rate alongside the pins, because a wrong rate is otherwise indistinguishable from a wiring fault: the sensor simply never answers. `doc/hw_todo.md`'s bring-up steps are corrected to 19200. The unused `SDF_SERVICES_DEFAULT_UART_*` block in `sdf_services.c` is removed — it was a second, silently unused set of defaults for the same settings.
...
### Added
- **The setup wizard asks for one fingerprint scan at a time (`add-wizard-enroll-scan-prompts`).** Enrolment takes three scans of the same finger, but nothing reported the scans individually: the wizard said "place your finger three times" and then showed an indeterminate spinner until the whole enrolment finished or failed. The enrolment executor now emits `ENROLLMENT_STEP_COMPLETE` on each captured scan, carrying the captured count and the scan now expected (a *retried* scan captured nothing and emits nothing, so the sequence never moves backwards). `sdf_ble_companion` notifies it as `{"status":"progress","captured":C,"step":N,"total":T}`; when the enrolment was started by a user-management request the notification carries that request id but **does not consume it**, since the terminal reply is still owed. The companion marks each confirmed scan and prompts for the next by number, in both the wizard and the dashboard's Enroll Fingerprint panel; markers state their status in text, so the count is not conveyed by colour alone.
- Fixing that exposed a second gap: `sdf_ble_companion_notify_enroll()` required an authenticated connection, so the first-time wizard — which enrols the first Admin *before* any account exists — could never receive any enrolment notification, terminal ones included. Enrolment notifications now also reach the setup connection over the User-Management characteristic while the setup phase is armed. Keyed on the phase being armed rather than on the setup state, because the state advances to `ADMIN_ENROLLED` at the moment the success notification is emitted; gating on state would exclude the wizard exactly when it needs the message.
- `SDF_EVENT_ROUTER_SUBS_BLE_COMPANION` goes 3 → 4 (production total 23 → 24) for the new subscription; the router pool is sized with zero headroom on purpose.
