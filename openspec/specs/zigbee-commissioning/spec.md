...
## Purpose
Defines the Zigbee commissioning and network membership tracking contracts, including network-joined state recording on all startup paths (factory-new network steering vs. reboot onto an existing network) and geometric backoff behavior on failed steering attempts to protect battery life.
