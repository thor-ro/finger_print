# OTA Session Lifecycle

## Purpose
Specifies how an `sdf_ota` session's resources — the digest context, the underlying `esp_ota_handle_t`, and the session's active flag — are acquired and released across success and failure paths, so that a failed OTA attempt never leaves the device unable to accept a subsequent one.
