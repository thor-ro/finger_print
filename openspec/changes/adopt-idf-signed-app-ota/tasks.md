...
- [ ] 8.5 Verify on a branch push that all three jobs run in parallel and that a deliberately broken signature fixture turns the new check red. Blocked: depends on 8.4, and a branch push is outside this session's scope (no push/commit without explicit request).  **Superseded by `add-ble-ota-emulator-harness`, whose task 5.5 is verified on GitHub Actions: green run `32726013619` (all three jobs parallel and green) and red run `32725675252` (deliberately broken fixture; only the `ota-signature` job failed while the other two stayed green).**
