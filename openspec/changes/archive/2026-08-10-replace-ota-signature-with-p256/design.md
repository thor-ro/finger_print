...
**Correction (change `harden-ota-commit-path`, 2026-08-10).** The last sentence above claimed more than it should have. D3's streaming digest is correct under flash encryption — that part stands, and was later confirmed on the emulator. But the *digest* was not the only thing the commit path read back from the target partition: `sdf_ota_verify_and_commit()` also read the 68-byte footer (via `sdf_ota_verify_signature()`) and the `esp_app_desc_t`, both while the `esp_ota_handle_t` was still open. Those two reads hit exactly the deferral this paragraph describes. Since app images are 16-byte aligned and the footer is 68 bytes, `total % 16 == 4` — the deferred slice is precisely the `SDF\x01` magic, so with flash encryption enabled every OTA would have failed with `Signature magic marker not found`. Scope this paragraph's correctness claim to the digest; see `harden-ota-commit-path` for the footer and descriptor reads, which move to stream capture for the same reason the digest already had.
