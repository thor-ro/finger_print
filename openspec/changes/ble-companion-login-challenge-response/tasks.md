...
- [x] 6.1 Benchmark `mbedtls_pkcs5_pbkdf2_hmac` on-device at the OWASP-baseline iteration count (~210k) to confirm one-time REGISTER-side cost is acceptable; adjust the compile-time iteration constant if not - **measured on real ESP32-C6 hardware** (standalone disposable benchmark project calling the exact `mbedtls_pkcs5_pbkdf2_hmac_ext` invocation used by `sdf_services_web_auth_stretch_credential`): 210,000 iterations took **17,748ms (~17.75s)**. This is unacceptable: the call has no internal yield points, so it fully blocks the sole RISC-V core for its entire duration, starving the CPU0 idle task well past the default 5s task-watchdog timeout and tripping a watchdog panic/reset before REGISTER can even complete - confirmed by reproducing the panic on-device before the watchdog was disabled for the timing run. Measured rate ≈84.5µs/iteration was used to pick a safer value: **10,000 iterations (NIST SP 800-132 minimum)**, independently confirmed on-device at **845ms**, comfortably under the watchdog timeout with margin, and a reasonable one-time REGISTER-side wait. `SDF_SERVICES_WEB_AUTH_PBKDF2_ITERATIONS` (`sdf_services.h`) adjusted from `210000u` to `10000u` accordingly, with rationale recorded in the header comment; this is a one-line, wire-protocol-shape-neutral change per design.md
