...
- Support firmware updates by passing Wi-Fi credentials over BLE, prompting the ESP32-C6 to download the OTA via Wi-Fi directly to avoid BLE MTU limits and battery drain.
...
- `sdf_app` / `sdf_protocol_ble`: Will host a new concurrent BLE GATT server alongside the existing Nuki BLE client.
