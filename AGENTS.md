...
The companion is a SvelteKit + Vite single-page app compiled to prerendered static assets and deployed to GitHub Pages (`https://thor-ro.github.io/finger_print/`) by `.github/workflows/deploy-web-companion.yml`. Node version is pinned in `.nvmrc`; `npm ci` from the committed lockfile; `npm run gate` chains the CI gates (svelte-check, lint, theme checks, Vitest, gzip bundle budget from `budget.json`). Key invariants enforced by `web-companion/scripts/lint.mjs`: no `{@html}` under `src/`, no colour literals under `src/` (all colours come from the theme tokens in `web-companion/themes/`, see its CONTRACT.md), `src/lib/protocol/` stays DOM-free, and `navigator.bluetooth` may only appear in `src/lib/transport/ble.ts`. Theme files (`web-companion/themes/*.css`) may declare tokens only — purity, token completeness and WCAG contrast are gated by `scripts/check-themes.mjs`. `scripts/csp.mjs` (post-build) pins the SHA-256 of every inline script (the pre-paint theme script and SvelteKit's bootstrap) into the CSP meta; production builds need `BASE_PATH=/finger_print`. See `web-companion/README.md`.
...
Bumble-based GATT central that pairs (LE SC Just Works), registers/logs in, and drives the companion BLE OTA protocol against the `ble_ota_gate` fixture (which opens the pairing window at boot) under `esp-emu --ble-hci tcp:`. Verified live through login and access-control rejection; **bulk image transfer is currently blocked by an esp-emu defect** (~28–31 inbound HCI ACL packets per boot wedge its BLE pipeline silently, esp-emu 0.39.0–0.40.1), so signature-case transfer over BLE is covered on hardware (`adopt-idf-signed-app-ota` tasks 6.2–6.7) and Layer 1 remains the required gate. See `openspec/changes/add-ble-ota-emulator-harness/design.md` D6 "Outcome" for the full evidence and the four stock-Bumble patches the bridge carries.

```bash
python3 -m venv /tmp/bleh_venv && /tmp/bleh_venv/bin/pip install -r tools/ble_ota_harness/requirements.txt
BLE_OTA_HARNESS_SCRATCH=/tmp/bleh_run BUMBLE_PYTHON=/tmp/bleh_venv/bin/python scripts/run_ble_ota_harness.sh
# The harness must be listening before esp-emu dials in (the runner handles this).
```
